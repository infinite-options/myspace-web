...
export default function TenantOnBoardingForm({ profileData, setIsSave }) {
	...
	const handleTaxIDChange = (value, onchangeflag) => {
		...
		if (onchangeflag) {
			...
			updateModifiedData({
				key: 'tenant_ssn',
				value: AES.encrypt(updatedTaxID, process.env.REACT_APP_ENKEY).toString(),
			});
		}
	};
		...
		try {
			...
			setSsn(
				profileData.tenant_ssn
					? AES.decrypt(profileData.tenant_ssn, process.env.REACT_APP_ENKEY).toString(CryptoJS.enc.Utf8)
					: ''
			);
			setMask(
				profileData.tenant_ssn
		...
		return parsedPaymentMethods.map((method, index) => (
			...
			>
							...
							<>
								...
								<Grid item xs={4}>
									<TextField
										...
										onChange={(e) =>
											handleChangeValue(e, method.paymentMethod_uid, 'routing_number')
										}
										variant="filled"
										fullWidth
				...
				{method.paymentMethod_uid && !method.paymentMethod_uid.startsWith('new_') && (
					<Grid item xs={1}>
						<IconButton
							onClick={() => handleDeletePaymentMethod(method.paymentMethod_uid)}
							aria-label="delete"
						>
							<DeleteIcon />
						</IconButton>
	...
	const handleChangeChecked = (e, uid) => {
		...
		const updatedMethods = parsedPaymentMethods.map((method) =>
			method.paymentMethod_uid === uid
				? { ...method, checked, paymentMethod_status: checked ? 'Active' : 'Inactive' }
				: method
		);
	...
	const handleChangeValue = (e, uid, field) => {
		...
		const updatedMethods = parsedPaymentMethods.map((method) =>
			method.paymentMethod_uid === uid ? { ...method, [`paymentMethod_${field}`]: value } : method
		);
		setParsedPaymentMethods(updatedMethods);
		setModifiedPayment(true);
	};
		...
		validPaymentMethods.forEach((method) => {
			...
			if (existingMethod) {
				const hasChanged =
					...
					(method.paymentMethod_type === 'bank_account' &&
						(method.paymentMethod_routing_number !== existingMethod.paymentMethod_routing_number ||
							method.paymentMethod_account_number !== existingMethod.paymentMethod_account_number));
	...
	const handleNextStep = async () => {
		...
		if (
			(taxIDType === 'EIN' && !DataValidator.ein_validate(ssn)) ||
			(taxIDType === 'SSN' && !DataValidator.ssn_validate(ssn))
		) {
		...
		try {
			if (modifiedData.length > 0) {
				...
				modifiedData.forEach((item) => {
					...
					//   }
					// } else {
					if (item.key !== 'tenant_income_1' &&
          item.key !== 'tenant_employment' &&
          item.key !== 'tenant_adult_occupants' &&
          ...
          item.key !== 'tenant_vehicle_info') {
				...
				await axios.put(
					'https://l0h6a9zi1e.execute-api.us-west-1.amazonaws.com/dev/profile',
					profileFormData,
					headers
				);
				setIsDialogOpen(true);
	...
	const handleIncomeChange = (event, index, field) => {
		...
		const hasChanges = employmentList.some(
			(emp, i) =>
			...
			>
				<Grid item xs={12}>
					<Typography
						align="center"
						gutterBottom
						sx={{ fontSize: '24px', fontWeight: 'bold', color: '#1f1f1f' }}
					>
								...
								<Grid container item xs={6}>
									...
									<Grid item xs={12}>
										<TextField
											...
											InputProps={{
												className:
													errors.phoneNumber || !phoneNumber ? classes.errorBorder : '',
											}}
											required
										></TextField>
									</Grid>
								...
								<Grid container item xs={3}>
									...
									<Grid item xs={12}>
										<TextField
											fullWidth
											value={licenseExp}
											onChange={handleLicenseExpChange}
											variant="filled"
											placeholder="License Exp"
											className={classes.root}
										></TextField>
									</Grid>
								</Grid>
                    ...
                    <MenuItem value='EIN'>EIN</MenuItem>
										...
										<RadioGroup
											...
											name="announctax_id_typeementType"
											value={taxIDType}
			...
			>
				<Grid item xs={12}>
					<Accordion
						...
						onChange={() => setPaymentExpanded((prevState) => !prevState)}
					>
						<AccordionSummary
          ...
          <Grid item xs={12}>
			...
			>
				<Grid item xs={12}>
					<Accordion
						sx={{ backgroundColor: '#F0F0F0', boxShadow: 'none' }}
						expanded={empinfoExpanded}
						onChange={() => setEmpinfoExpanded((prevState) => !prevState)}
					>
						<AccordionSummary
							expandIcon={<ExpandMoreIcon />}
							aria-controls="income-details-content"
							id="income-details-header"
						>
							<Grid container justifyContent="center">
								<Grid item md={11.5}>
									<Typography
			...
			>
				<Grid item xs={12}>
					<Accordion
						sx={{ backgroundColor: '#F0F0F0', boxShadow: 'none' }}
						expanded={occupancyExpanded}
						onChange={() => setOccupancyExpanded((prevState) => !prevState)}
					>
						<AccordionSummary
							expandIcon={<ExpandMoreIcon />}
