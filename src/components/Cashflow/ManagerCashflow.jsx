...
export default function ManagerCashflow() {
  ...
  const [unsortedPayouts, setUnsortedPayouts] = useState(null);
  const [unsortedRentsData, setUnsortedRentsData] = useState(null);
  ...
  const getRevenueByType = (data, month, year, expected) => {
    ...
    // );
    let revenueItems = data?.filter(
      (item) => item.pur_cf_type === "revenue" && item.cf_month === month && item.cf_year === year && item.purchase_type.toUpperCase() !== "DEPOSIT"
    );
  ...
  const getTotalByType = (data, month, year, expected, sorting) => {
    ...
    if (sorting) {
      ...
      sortedTotals = Object.fromEntries(nonZeroTotals.sort(([keyA], [keyB]) => keyA.localeCompare(keyB)));
    }
  ...
  useEffect(() => {
    ...
    if(cashflowData){
      ...
      if (selectedOwnerId !== "All Owner") {
        ...
        const finalList = Object.values(groupedByPurGroup).filter((group) => group.some((item) => item.pur_payer === selectedOwnerId || item.pur_receiver === selectedOwnerId));
      ...
      const currentYearData = filteredProfitData?.filter((item) => item.cf_year === year);
      ...
      const rentsDataByPropertyCurrentYear = rentDataCurrentYear?.reduce((acc, item) => {
        ...
        const expectedProfit = parseFloat(item.pur_amount_due ? item.pur_amount_due : 0.0) || 0;
  ...
  //     //   const sortedProperties = Object.entries(monthData.properties).sort(([uidA], [uidB]) => uidA.localeCompare(uidB)); // Sort by property_uid
  //     //   return [
  //     //     month,
  //     //     {
  //     //       ...monthData,
  ...
  const calculateProfitsByMonth = (rentsDataByMonth, payoutsByMonth) => {
    ...
    Object.keys(rentsDataByMonth).forEach((monthYearKey) => {
      ...
      Object.keys(rentDataForMonth.properties).forEach((propertyUID) => {
        ...
        // Filter rent items
        const profitRentItems = rentData.profitItems.filter((item) => parseFloat(item.pur_amount_due ? item.pur_amount_due : 0.0) >= 0 || parseFloat(item.total_paid) >= 0);

        const profitPayoutItems = payoutData.profitItems
          .filter((item) => parseFloat(item.pur_amount_due ? item.pur_amount_due : 0.0) >= 0 || parseFloat(item.total_paid) >= 0)
          .map((item) => ({
            ...item,
            pur_amount_due: (parseFloat(item.pur_amount_due ? item.pur_amount_due : 0.0) || 0) * -1,
