...
console.log("process.env.REACT_APP_ENCRYPTION_ON - ", process.env.REACT_APP_ENCRYPTION_ON);
const ENCRYPTION_ENABLED = process.env.REACT_APP_ENCRYPTION_ON != null ? process.env.REACT_APP_ENCRYPTION_ON === "true" : false;
...
function decryptPayload(encryptedBlob) {
  try {
    ...
    const ciphertext = CryptoJS.lib.WordArray.create(encryptedData.words.slice(BLOCK_SIZE / 4), encryptedData.sigBytes - BLOCK_SIZE);
...
// Note: Endpoint selection is now handled in Endpoints.js

const getApiUrl = (url) => {
  // URL transformation is no longer needed - endpoints are now managed in Endpoints.js
  return url;
};
  ...
  try {
    ...
    if (response.status === 404) {
      ...
      if (response.status === 401) {
        ...
        const newAccessToken = refreshData.encrypted_data ? decryptPayload(refreshData.encrypted_data).access_token : refreshData.access_token;
    ...
    if (error.response?.status === 404) {
      ...
      if (error.response?.status === 401 && !originalRequest._retry) {
        ...
        try {
          ...
          const newAccessToken = refreshResponse.data.encrypted_data ? decryptPayload(refreshResponse.data.encrypted_data).access_token : refreshResponse.data.access_token;
...
export { axiosMiddleware, fetchMiddleware };
